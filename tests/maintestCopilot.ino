...
void edgeFound(){
    if(isOnEdge()) {
        ...
        while (millis() - startMillis < 1000) {
            // Espera 1 segundo sem bloquear o loop principal
        }
        moveBackward(0.4);
        startMillis = millis();
        while (millis() - startMillis < 1000) {
        ...
        while (millis() - startMillis < 100) {
            // Espera 100 ms sem bloquear o loop principal
        }
    }
}
...
// Funções de controle dos motores (substitua com a lógica correta)
void moveForward(float speed) {
    analogWrite(MOTOR_1_R, speed * 255);
    analogWrite(MOTOR_1_L, 0);
    analogWrite(MOTOR_2_R, speed * 255);
    analogWrite(MOTOR_2_L, 0);
}

void turnLeft(float speed) {
    analogWrite(MOTOR_1_R, 0);
    analogWrite(MOTOR_1_L, speed * 255);
    analogWrite(MOTOR_2_R, speed * 255);
    analogWrite(MOTOR_2_L, 0);
}
...
void turnRight(float speed) {
    ...
    analogWrite(MOTOR_2_L, speed * 255);
}
